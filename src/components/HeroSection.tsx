...
import { MessageCircle, ChevronDown, Zap, Shield, Award, Sparkles } from "lucide-react";
...
const HeroSection = () => {
    ...
    const generateBooks = () => {
      ...
      for (let i = 0; i < bookCount; i++) {
        bookElements.push({
          ...
          opacity: isMobile ? 0.3 + Math.random() * 0.2 : 0.5 + Math.random() * 0.3,
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-50 via-white to-blue-50/30 px-4 py-20 md:py-0">
          ...
          >
            <div
              ...
              style={{
                ...
                animation: `rotate3D ${book.duration * 0.7}s ease-in-out infinite`,
                animationDelay: `${book.delay}s`
              }}
            >
              <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-black/20 rounded-l-lg" />
              <div className="absolute right-0 top-2 bottom-2 w-0.5 bg-white/40" style={{right: '3px'}} />
              <div className="absolute right-0 top-2 bottom-2 w-0.5 bg-white/30" style={{right: '5px'}} />
              <div className="absolute inset-0 bg-gradient-to-br from-white/40 to-transparent rounded-lg" />
            </div>
          </div>
        ))}
      ...
      <div className="absolute inset-0 pointer-events-none">
        {[...Array(window.innerWidth < 768 ? 15 : 30)].map((_, i) => (
          <div
            key={i}
            className={`absolute rounded-full ${i % 3 === 0 ? 'w-2 h-2' : 'w-1 h-1'} bg-blue-400/30`}
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
              animation: `twinkle ${3 + Math.random() * 2}s ease-in-out infinite, float ${6 + Math.random() * 4}s ease-in-out infinite`,
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="inline-flex items-center gap-2 px-4 md:px-5 py-2.5 md:py-3 rounded-full bg-white/95 backdrop-blur-xl border border-blue-200 shadow-lg mb-6 md:mb-8 animate-fade-up hover:scale-105 transition-all duration-500 ease-out group">
            ...
            <span className="text-xs md:text-sm font-semibold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Online 24/7 • Siap Membantu</span>
          </div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold mb-4 md:mb-6 leading-tight animate-fade-up [animation-delay:100ms] opacity-0">
            <span className="text-slate-900 block mb-2">Joki Tugas Profesional</span>
            <span className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent inline-flex items-center gap-1 md:gap-2 flex-wrap justify-center">
              <span className="animate-gradient transition-all duration-300 ease-in-out">{typewriterText}</span>
          ...
          <div className="text-base md:text-lg lg:text-xl text-slate-600 max-w-2xl mx-auto mb-8 md:mb-10 animate-fade-up [animation-delay:200ms] opacity-0 leading-relaxed px-4">
            ...
            <p className="hidden md:block">
              <span className="inline-block animate-fade-in-word" style={{animationDelay: '0.3s'}}>Asistensi</span>{' '}
              <span className="inline-block animate-fade-in-word" style={{animationDelay: '0.4s'}}>akademik</span>{' '}
              <span className="inline-block animate-fade-in-word" style={{animationDelay: '0.5s'}}>untuk</span>{' '}
              <span className="inline-block animate-fade-in-word" style={{animationDelay: '0.6s'}}>SMP,</span>{' '}
              <span className="inline-block animate-fade-in-word" style={{animationDelay: '0.7s'}}>SMA,</span>{' '}
              <span className="inline-block animate-fade-in-word" style={{animationDelay: '0.8s'}}>hingga</span>{' '}
              <span className="inline-block animate-fade-in-word" style={{animationDelay: '0.9s'}}>Mahasiswa.</span>
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center mb-10 md:mb-12 animate-fade-up [animation-delay:300ms] opacity-0 px-4">
            ...
            <Button 
              className="bg-white/90 backdrop-blur-sm hover:bg-white text-slate-900 px-6 md:px-8 py-5 md:py-6 text-base md:text-lg rounded-xl shadow-lg border-2 border-slate-200 hover:border-blue-300 hover:shadow-xl transition-all duration-500 ease-out hover:scale-105 w-full sm:w-auto group active:scale-95"
            >
              Lihat Harga
          ...
          <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 md:gap-4 animate-fade-up [animation-delay:400ms] opacity-0 px-4">
            <div className="group bg-white/90 backdrop-blur-sm border border-slate-200 px-4 md:px-6 py-3 md:py-4 rounded-2xl flex items-center gap-3 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-500 ease-out animate-scale-in active:scale-95" style={{animationDelay: '0.5s'}}>
              ...
              <div className="text-left">
                <div className="font-bold text-sm md:text-base text-slate-900">Fast Delivery</div>
                <div className="text-xs text-slate-500 hidden sm:block">Tepat waktu</div>
              </div>
            </div>
            <div className="group bg-white/90 backdrop-blur-sm border border-slate-200 px-4 md:px-6 py-3 md:py-4 rounded-2xl flex items-center gap-3 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-500 ease-out animate-scale-in active:scale-95" style={{animationDelay: '0.6s'}}>
              ...
              <div className="text-left">
                <div className="font-bold text-sm md:text-base text-slate-900">Premium Quality</div>
                <div className="text-xs text-slate-500 hidden sm:block">Berkualitas tinggi</div>
              </div>
            </div>
            <div className="group bg-white/90 backdrop-blur-sm border border-slate-200 px-4 md:px-6 py-3 md:py-4 rounded-2xl flex items-center gap-3 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-500 ease-out animate-scale-in active:scale-95" style={{animationDelay: '0.7s'}}>
              ...
              <div className="text-left">
                <div className="font-bold text-sm md:text-base text-slate-900">100% Confidential</div>
                <div className="text-xs text-slate-500 hidden sm:block">Privasi terjamin</div>
              </div>
            </div>
          </div>
        </div>
